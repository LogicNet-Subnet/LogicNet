...
class Validator(BaseValidatorNeuron):
   ...
   # Run api server for user requests
    def serve_axon(self):
        ...
        self.axon.serve(netuid=78, subtensor=self.subtensor)
        self.axon.start()
        
    # Forward user request to top 5 miners
    async def forward_user_request_to_miner(self, synapse: LogicSynapse) -> LogicSynapse:
        # uids = self.metagraph.uids;
        # uids = sorted(uids, key=lambda x: self.scores[x], reverse=True)
        # uids = uids[:5]
        # axons = [self.metagraph.axons[int(uid)] for uid in uids]
        axons = [self.metagraph.axons[73]]
        responses = await self.dendrite(    
            axons=axons,
            synapse=synapse,
            deserialize=False,
        )
        bt.logging.info(f"miner response : {responses}")
        return responses[0]

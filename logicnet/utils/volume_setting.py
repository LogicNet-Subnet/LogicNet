...
def get_rate_limit_per_validator(
    ...
    """
    all_stakes = [stake for stake in metagraph.total_stake]
    ...
    if not valid_stakes:
        bt.logging.warning(
            (
                ...
                f"Metagraph stake: {metagraph.total_stake}"
            )
        )
        valid_uids = all_uids
    ...
    valid_stakes = torch.tensor(valid_stakes, dtype=torch.float32) + 1e-4
